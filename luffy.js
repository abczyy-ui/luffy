...
// ════════════════════════════════════════════════════════════════════════════
const CONFIG = {
  botPhone    : "6285165639635",   // Nomor WA BOT
  ownerNumber : "6283171413750",   // Nomor WA owner
  ownerLID    : "155418206691577", // LID owner
...
const log = {
  ...
  battle : (m) => console.log(`${C.dim}[${ts()}]${C.reset} ${C.red}[RPG]${C.reset}    ${m}`),
...
function resolveOwnerInfo(msg) {
  ...
  const isOwner =
    senderNum === CONFIG.ownerNumber ||
    senderNum === CONFIG.ownerLID    ||
    jidNum    === CONFIG.ownerNumber ||
    jidNum    === CONFIG.ownerLID;
...
const hpBar  = (cur, max, len = 12) => {
  const fill = Math.max(0, Math.round((cur / max) * len));
  return "█".repeat(fill) + "░".repeat(len - fill);
};

function getAvailableChar(db) {
  const taken = Object.values(db.players).map((p) => p.character);
  const avail = Object.keys(CHARACTERS).filter((c) => !taken.includes(c));
  return avail.length ? rand(avail) : null;
}

function getEnemy(danger) {
  if (danger <= 3) return rand(ENEMIES.easy);
  if (danger <= 7) return rand(ENEMIES.medium);
  return rand(ENEMIES.hard);
}

function calcLevel(exp) {
  const level = Math.floor(exp / 500) + 1;
  const titles = [
    "Bajak Laut Pemula","Kru Biasa","Perwira Bajak Laut",
    "Kapten Berburu","Bajak Laut Terkenal","Shichibukai",
    "Yonko Commander","Yonko","Raja Bajak Laut",
  ];
  return { level, rank: titles[Math.min(Math.floor(level / 10), titles.length - 1)] };
}
...
async function handleMessage(sock, msg) {
  ...
  log.chat(
    `${C.bright}${pushName}${C.reset} [${senderNum}]` +
    (isOwner  ? ` ${C.magenta}👑OWNER${C.reset}` : "") +
    (isGroup  ? ` ${C.cyan}[GRP]${C.reset}` : ` ${C.dim}[PM]${C.reset}`) +
  ...
  if (text === ".regist") {
    ...
    if (!charKey) {
      await sock.sendMessage(jidRaw, {
        text: `${mention(senderNum)} Maaf, semua slot karakter penuh!`,
        mentions: [`${senderNum}@s.whatsapp.net`],
      });
      return;
    }
    const ch = CHARACTERS[charKey];
    db.players[senderNum] = {
      character: charKey, hp: ch.hp, maxHp: ch.hp,
      exp: 0, berry: 10000, locationIndex: 0,
      adventure: false, currentEnemy: null,
      wins: 0, defeats: 0, specialUsed: 0,
  ...
  if (text === ".start") {
    ...
    if (!db.players[senderNum]) {
      await sock.sendMessage(jidRaw, {
        text: `🏴‍☠️ ${mention(senderNum)} kamu belum terdaftar!\nKetik *.regist* dulu ya.`,
        mentions: [`${senderNum}@s.whatsapp.net`],
      });
      return;
    }
    ...
    p.adventure    = true;
    p.currentEnemy = { ...enemy, currentHp: enemy.hp };
    saveDB(db);
  ...
  if (text === ".attack" || text === ".attack spesial") {
    ...
    const p   = db.players[senderNum];
    const ch  = CHARACTERS[p.character];
    if (!p.adventure || !p.currentEnemy) {
      await sock.sendMessage(jidRaw, {
        text: `⚓ ${mention(senderNum)} kamu belum dalam petualangan!\nKetik *.start* untuk mulai.`,
        mentions: [`${senderNum}@s.whatsapp.net`],
      });
      return;
    }
    ...
    let pdmg, atkDesc;
    if (isSpecial) {
      pdmg    = Math.floor(ch.attack * ch.special_multiplier * (0.9 + Math.random() * 0.2) - enemy.defense * 0.5);
      pdmg    = Math.max(pdmg, 50);
      atkDesc = `✨ *${ch.special}!*`;
      p.specialUsed = (p.specialUsed || 0) + 1;
    } else {
      pdmg    = Math.floor(ch.attack * (0.8 + Math.random() * 0.4) - enemy.defense * 0.3);
      pdmg    = Math.max(pdmg, 20);
      atkDesc = `⚔️ *${rand(["Serangan telak menghantam keras!","Pukulan cepat mengenai musuh!","Tendangan kuat menghantam target!","Serangan beruntun melukai musuh!"])}*`;
    }
...
// ════════════════════════════════════════════════════════════════════════════
//  KONEKSI WHATSAPP — FIXED: pairing code request timing diperbaiki
// ════════════════════════════════════════════════════════════════════════════
async function connectToWhatsApp() {
  ...
  // ── FIX: Auto pairing code — request setelah socket siap ──
  if (!sock.authState.creds.registered) {
    log.info(`Nomor bot: ${CONFIG.botPhone}`);
    log.info("Menunggu socket siap sebelum request pairing code...");
    ...
    const tryPairing = async () => {
      ...
      try {
        ...
        log.warn("Retry pairing dalam 5 detik...");
        pairingDone = false;
        setTimeout(tryPairing, 5000);
      }
    };

    // Tunggu 3 detik lalu langsung request — jangan tunggu event "connecting"
    setTimeout(tryPairing, 3000);
  }
  ...
  sock.ev.on("connection.update", ({ connection, lastDisconnect }) => {
    if (connection === "close") {
      ...
      if (retry) {
        ...
        log.warn("Logged out! Hapus folder auth_info lalu jalankan ulang.");
      }
