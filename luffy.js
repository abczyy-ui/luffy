...
// ════════════════════════════════════════════════════════════════════════════
//  KONEKSI WHATSAPP — FIXED: tanpa readline, nomor hardcode, auto pairing
// ════════════════════════════════════════════════════════════════════════════
async function connectToWhatsApp() {
  ...
  // ── Auto pairing code tanpa readline ──
  if (!sock.authState.creds.registered) {
    log.info(`Nomor bot: ${CONFIG.botPhone}`);
    log.info("Menunggu koneksi sebelum request pairing code...");
    ...
    sock.ev.on("connection.update", async ({ connection }) => {
      if (connection === "connecting" && !pairingDone) {
        pairingDone = true;
        // Tunggu 6 detik agar socket siap
        ...
        try {
          ...
          log.warn("Tunggu 10 detik lalu coba ulang...");
          setTimeout(() => {
            pairingDone = false;
          }, 10000);
        }
